...
public class StageService extends BaseService {
    ...
    public void unlockStage(ReqMainPVEUnlockData reqMainPVEUnlock) {
        ...
        List<Item> itemList = rewardManager.reward(player, Numerics.get(StageNumeric.class, stageId).get().getRewards(), new float[]{levelId});

        RespMainPVEUnlockData respMainPVEUnlock = RespMainPVEUnlockData.builder().nextStageId(nextStageId).nextLevelId(nextLevelId).itemList(itemList).build();
